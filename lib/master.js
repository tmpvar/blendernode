  ...
  skateboard({
    ...
    requestHandler : function(req, res) {
      ...
      if (req.method.toLowerCase() === 'post' && req.url.substring(0,4) === '/job') {
        ...
        var blendFile = path.resolve(__dirname + '/../tmp/test-' + filename);
        console.log("creating file:", blendFile)


        const writer = req.pipe(fs.createWriteStream(blendFile))
        ...
        writer.on('finish', function() {
          ...
          blendClient.putStream(fs.createReadStream(blendFile), filename, req.headers, function(e) {
            if (e) {
              res.writeHead(500);
              return res.end('{}');
            }
          ...
          blender.on('exit', function(code) {
            const line = out.split("\n").filter((line) => {
              return line[0] == '{'
            }).pop()

            console.log(line)
            blends[filename] = JSON.parse(line);
            createSlaves();
          });
        });
